import os
from .utils import hex_a_ass_color, generar_color_por_intensidad, redondear_tiempo, segundos_a_ass, segundos_a_mmss, limpiar_override, wrap_text
...
def css_to_ass_style(name, css, d_font, d_color):
    ...
    # BorderStyle=1, Outline=0, Shadow=0, Alignment=5 (Center) - Look limpio tipo script19.py
    return f"Style: {name},{font},{fs},{ass_color},&H00FFFFFF,&H00FFFFFF,&H00FFFFFF,{bold},{italic},0,0,100,100,0,0,1,0,0,5,10,10,10,1"
...
def generar_subtitulos_ass(meta, styles, segments, out_file, debug_mode=False):
    ...
    # Resolver fuente: usa la del meta del guíon, pero valida con FontResolver
    fr = get_resolver()
    font_name = meta.get('MAIN_FONT', fr.get_font_name())

    header = f"[Script Info]\nScriptType: v4.00+\nPlayResX: {W}\nPlayResY: {H}\nScaledBorderAndShadow: yes\n\n[V4+ Styles]\nFormat: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding\n"

    header += f"Style: Default,{font_name},54,&H00000000,&H00FFFFFF,&H00FFFFFF,&H00FFFFFF,0,0,0,0,100,100,0,0,1,0,0,5,10,10,10,1\n"

    if debug_mode:
        header += f"Style: Debug,{font_name},36,&H0000FFFF,&H00FFFFFF,&H00000000,&H80000000,1,0,0,0,100,100,0,0,1,2,1,9,20,20,20,1\n"
    
    for name, css in styles.items():
        header += css_to_ass_style(name, css, font_name, meta.get("MAIN_TEXT_COLOR","#000000")) + "\n"
    ...
    for i, seg in enumerate(texts):
        ...
        if style.upper() == "FORZADO":
            color_override = f"\\c{{hex_a_ass_color('#FFFF00')}}"
        elif style.upper() != "VIDEOSUB":
        ...
        if seg_type in ("SPLIT_LEFT", "SPLIT_RIGHT", "VIDEO", "FULL_IMAGE"):
            ...
            # Para pantallas TEXT puro: aumentamos la fuente un 50% y dejamos que el motor ASS
            # haga el salto de línea inteligente (Word Wrapping) aplicando grandes márgenes laterales.
            size_override = "\\fscx150\\fscy150"
            override = size_override + color_override
            txt = f"{{{override}}}{plain}"
            
            # Aplicar 15% del ancho de margen a cada lado (ej. ~288px en 1920x1080)
            margin_l = int(W * 0.15)
            margin_r = int(W * 0.15)
    ...
    with open(out_file, "w", encoding="utf-8") as f:
        f.write(header + "\n".join(ev_lines))
    print("Subtítulos ASS generados →", out_file)
    if debug_mode:
        print("  ⚠ Modo DEBUG activado: tiempos visibles en esquina superior derecha")
    return out_file
